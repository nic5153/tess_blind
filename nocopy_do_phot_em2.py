...
folders=($(eval "ls -d /lustre/research/mfausnau/data/tica/s{sector:04}/cam{cam}-ccd{ccd}/o??/slice*/"))
...
cmdlist = ["""echo 'Decompress kernel_data if it exists'1>&2""","""[[ -e 'kernel_data.tgz' ]] && { tar -xzf kernel_data.tgz; }""","""tarstat=$?""","""sleep 5""","""[[ -e psf_table ]] || { ln -s ../psf_table ; }""","""[[ -e process_config ]] || { ln -s ../process_config . ; }""","""for p in $(ls ../psf_file*fits) ; do [[ -e ${p:3:30} ]] || { ln -s $p ; } ; done ""","""echo 'tar exit status' ${tarstat}""","""echo 'Run phot2.csh'""","""${ISIS_DIR}/phot2.csh""","""echo 'tar kernel_data.tgz if it does not exist'""","""[[ -e 'kernel_data.tgz' ]] || tar -czf kernel_data.tgz {kt_,kc_}*fits""","""sleep 5""","""[[ -e 'bkg_phot' ]] || { mkdir bkg_phot ; cd bkg_phot; ${PIPELINE_DIR}/setup/make_bkg_phot_dir_em2; cd .. ; } ""","""cd bkg_phot""","""[[ -e 'lcGRB' ]] && { rm -rf lcGRB ; } ""","""mkdir lcGRB""","""echo 'Decompress kernel_data if it exists'1>&2""","""[[ -e 'kernel_data.tgz' ]] && { tar -xzf kernel_data.tgz; }""","""tarstat=$?""","""sleep 5""","""[[ -e psf_table ]] || { ln -s ../psf_table ; }""","""[[ -e process_config ]] || { ln -s ../process_config . ; }""","""for p in $(ls ../psf_file*fits) ; do [[ -e ${p:3:30} ]] || { ln -s $p ; } ; done ""","""echo 'tar exit status' ${tarstat}""","""echo 'Run phot2.csh'""","""${ISIS_DIR}/phot2.csh""","""[[ -e 'kernel_data.tgz' ]] || tar -czf kernel_data.tgz {kt_,kc_}*fits""","""[[ -e 'kernel_data.tgz' ]] && { rm -rf kt_*fits kc_*fits; }""","""cd ..""","""[[ -e 'kernel_data.tgz' ]] && { rm -rf kt_*fits kc_*fits; }"""]
...
cmdlist = ["""source /home/nimcclur/scratch/rms_testing/do_phot_em2.sh""","""set +euo""",f"""copy_phot {sector:02} {cam} {ccd}""",f"""clean_phot {sector:02} {cam} {ccd}"""]
